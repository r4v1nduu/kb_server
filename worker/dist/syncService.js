...
class SyncService {
    constructor(esService, mongoService) {
        this.esService = esService;
        this.mongoService = mongoService;
    }
    mapEmailToESPayload(email) {
        return {
    ...
    async handleEmailChange(change) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                switch (change.operationType) {
                    ...
                    case "replace":
                        if (change.fullDocument) {
                            ...
                            console.log(`[INFO] Synced ${change.operationType} for email ${documentId}`);
                        }
                        break;
                    case "delete":
                ...
                console.error(`[BAD] Attempt ${attempt}/${MAX_RETRIES}: Error syncing email ${documentId}:`, error);
                if (attempt === MAX_RETRIES) {
                    console.error(`[FATAL] Failed to sync email ${documentId} after ${MAX_RETRIES} attempts.`);
                }
                else {
        ...
        try {
            ...
            for (let i = 0; i < emails.length; i += BATCH_SIZE) {
                ...
                try {
                    ...
                    console.error(`[BAD] Error syncing batch starting at index ${i}:`, error);
                }
            }
            console.log(`[INFO] Initial sync completed: ${synced}/${emails.length} emails synced`);
        }
        catch (error) {
