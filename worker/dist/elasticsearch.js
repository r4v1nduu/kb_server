...
class ElasticsearchService {
    constructor(elasticsearchUrl) {
        this.indexName = "emaildb-email"; // Changed to prefixed naming
    ...
    async createIndexIfNotExists() {
        try {
            ...
            if (!exists) {
                const replicaCount = parseInt(process.env.ELASTICSEARCH_REPLICAS || "", 10) || 0;
    ...
    async deleteEmail(id) {
        try {
            ...
            if (error.meta?.statusCode === 404) {
                console.log(`[INFO] Email ${id} not found in Elasticsearch (already deleted)`);
                return;
            }
    ...
    async bulkIndexEmails(payloads) {
    ...
    async search(query, size = 50, from = 0) {
        try {
            const searchQuery = {
                index: this.indexName,
                size,
